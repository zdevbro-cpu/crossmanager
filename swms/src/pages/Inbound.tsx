...
import { TrendingDown, Plus, XCircle, Trash2 } from 'lucide-react'
...
export default function InboundPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // }
            const res = await apiClient.get(`/swms/inbounds?${params.toString()}`)
            setData(res.data)
        } catch (err: any) {
            console.error(err)
            show(err.response?.data?.error || '입고 데이터를 불러오지 못했습니다.', 'error')
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await apiClient.post('/swms/inbounds', payload)
            show('입고가 완료되었습니다. 재고가 업데이트되었습니다.', 'success')
            setShowModal(false)
            resetForm()
            fetchData()
        } catch (err: any) {
            console.error(err)
            show(err.response?.data?.error || '입고 등록 실패', 'error')
        } finally {
            setSaving(false)
        }
    }
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            await apiClient.delete(`/swms/inbounds/${id}`)
        ...
        <div className="page">
            <header className="page-header">
                <div>
                    ...
                    <h1>입고 관리</h1>
                    <p className="muted">{currentSite.name} - 자재 입고 내역을 관리합니다.</p>
                </div>
            ...
            {data.length === 0 ? (
                <section className="empty-state">
                    ...
                    <h3>등록된 입고 내역이 없습니다</h3>
                    <p>우측 상단의 "입고 등록" 버튼을 눌러 첫 입고를 기록하세요.</p>
                </section>
            ) : (
                    ...
                    <table className="table" style={{ tableLayout: 'fixed', width: '100%' }}>
                        ...
                        <tbody>
                            {data.map((row) => (
                                <tr key={row.id}>
                                    <td>
                                        {new Date(row.inbound_date).toLocaleDateString('ko-KR', { year: 'numeric', month: '2-digit', day: '2-digit' }).replace(/\. /g, '-').replace('.', '')}
                                    </td>
