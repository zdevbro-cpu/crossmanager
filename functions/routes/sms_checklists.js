    ...
    router.post('/submit', async (req, res) => {
        ...
        try {
            const {
                ...
                results,
                meta_info,
                // 점검을 실제로 한 날. 안 보내면 오늘로 둔다.
                // created_at(입력 시각)을 쓰면 어제 한 점검을 오늘 입력했을 때
                // 달력에 하루 뒤로 찍힌다.
                date
            } = req.body;
            ...
            const query = `
                INSERT INTO sms_checklists (
                    project_id, template_id, title, date,
                    status, results, immutable_snapshot,
                    created_by, created_at, submitted_at,
                    weather_info, location_info
                )
                VALUES ($1, $2, $3, COALESCE($9::date, CURRENT_DATE), 'SUBMITTED', $4, $5, $6, NOW(), NOW(), $7, $8)
                RETURNING *
            `;
