...
interface DocumentUploadModalProps {
    ...
    initialClient?: string // Add new prop
    onSuccess?: () => void
}

export default function DocumentUploadModal({ isOpen = true, onClose, initialProject, initialCategory, initialClient, onSuccess }: DocumentUploadModalProps) {
    if (!isOpen) return null

    const fileInputRef = useRef<HTMLInputElement>(null)
    const [file, setFile] = useState<File | null>(null)
    const [dragActive, setDragActive] = useState(false)
    const [attDragActive, setAttDragActive] = useState(false) // New state for attachments
    ...
    const [clientName, setClientName] = useState(initialClient || '') // Use prop or default
    ...
    // Attachment DnD Handlers
    const handleAttDrag = (e: React.DragEvent) => {
        e.preventDefault()
        e.stopPropagation()
        if (e.type === 'dragenter' || e.type === 'dragover') {
    ...
    const handleAttDrop = (e: React.DragEvent) => {
        e.preventDefault()
        e.stopPropagation()
        setAttDragActive(false)
                    ...
                    <div className="upload-split">
                        ...
                        <form className="meta-form" onSubmit={handleSubmit}>
                            ...
                            <div className="form-group">
                                ...
                                />
                                <div
                                    className={`attachment-drop-zone ${attDragActive ? 'active' : ''}`}
                                    onDragEnter={handleAttDrag}
                                    onDragLeave={handleAttDrag}
                                    onDragOver={handleAttDrag}
                                    onDrop={handleAttDrop}
                                    style={{
                                        border: attDragActive ? '1px dashed #4dabf7' : '1px dashed #495057',
                                        backgroundColor: attDragActive ? 'rgba(77, 171, 247, 0.1)' : 'transparent',
                                        borderRadius: '4px',
                                ...
                                >
                                    {attachments.length > 0 ? (
                                        <div className="attachment-preview-list" style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '4px' }}>
                                        ...
                                        <div style={{ padding: '8px', fontSize: '0.8rem', color: '#868e96', textAlign: 'center', pointerEvents: 'none' }}>
                                            {attDragActive ? '여기에 놓으세요!' : '파일을 드래그하거나 추가 버튼 클릭'}
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className={`client-section ${isClientSubmit ? 'active' : ''}`}>
                                ...
                                >
                                    ...
                                    <div className="label-group">
                                        <span className="main-label">발주처 제출용 문서</span>
