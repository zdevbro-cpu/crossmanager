...
const createDocumentsRouter = (pool, uploadsDir) => {
    ...
    // 5.1 Lock/Unlock (Checkout/Checkin) - NEW
    router.patch('/:id/lock', async (req, res) => {
        const { id } = req.params
        try {
            await pool.query('UPDATE documents SET is_checked_out = true, updated_at = NOW() WHERE id = $1', [id])
            res.json({ message: 'Document locked (Checked out)' })
        } catch (err) {
            console.error(err)
    ...
    router.patch('/:id/unlock', async (req, res) => {
        const { id } = req.params
        try {
            await pool.query('UPDATE documents SET is_checked_out = false, updated_at = NOW() WHERE id = $1', [id])
            res.json({ message: 'Document unlocked (Checked in)' })
        } catch (err) {
            console.error(err)
    ...
    // 5.2 Download Logic - Dedicated Redirect
    router.get('/:id/download', async (req, res) => {
        try {
            const { id } = req.params
            const query = `
                SELECT v.file_path FROM documents d
                LEFT JOIN document_versions v ON d.id = v.document_id AND d.current_version = v.version
                WHERE d.id = $1
            `
            const { rows } = await pool.query(query, [id])
            ...
            if (!filePath.startsWith('http') && !filePath.startsWith('/') && bucket) {
